...
{
	public IList<Timeline> ObjectTimelines = new List<Timeline>();

	public FrameRenderer(IList<Timeline> timelines) { ObjectTimelines = timelines; }

	public FrameRenderer() { }

	private BatchRenderer GenerateRenderer(out ulong frameCount)
	{
	...
	public SKImage[] RenderFrames(int width, int height)
	{
	...
	public void RenderFramesToDisk(int width, int height, string dir, EventHandler<(int, int)>? progress)
	{
		...
		var frameNames = Enumerable.Range(1, (int) frameCount)
								   .Select(i => Path.Combine(directory.FullName, i.ToString().PadLeft(6, '0') + ".png"))
								   .ToArray();

		renderer.RenderAllToFiles(width, height, frameNames, progress);
	}

	private void GenerateLayers(Renderer rend, ulong frame)
	{
	...
	private static (TimePoint?, TimePoint?) FindRelevantTimingPoints(ulong frameNum, Timeline timeline)
	{
